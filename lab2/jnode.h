...
namespace jora {
    ...
    public:
        ...
        inline const std::shared_ptr<Node<Type>>& next() noexcept { return _next; }
        inline const std::shared_ptr<const Node<Type>>& next() const noexcept { return _next; }
        inline void setNext(const std::shared_ptr<Node<Type>>& next) noexcept { _next = next; }
        inline void setNext(const Node<Type>& next) { _next = Node<Type>::create(next); }
    ...
    private:
        Type _value = Type();
        std::shared_ptr<Node<Type>> _next = std::shared_ptr<Node<Type>>();
    };
}
