...
public:
    ...
    CAddCamera(const std::weak_ptr<ObjectId>& resultId)
    ...
    virtual void execute(Composite& scene, Manager& manager) {
        ...
        if (result) {
            if (!_resultId.expired())
                *_resultId.lock() = result->id();
...
public:
    ...
    CAddModel(std::string&& filename, const std::weak_ptr<ObjectId>& resultId)
